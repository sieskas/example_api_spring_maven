...
class ExampleMapperTest {
    ...
    void toModel_valid() {
        // given
        List<ItemResource> itemResources = Arrays.asList(ItemResource.builder().name("name").type("BOOK").price("10.0").build(),
	...
	void toModel_invalid() {
		ExampleRequestResource exampleRequestResource =
				ExampleRequestResource.builder().numAccount("0001").items(Collections.singletonList(
						ItemResource.builder().name("name3").type("INVALID").price("30.0").build())).build();
