...
public class JsonPaceHolderAdapterConfig {
	@ConditionalOnProperty(
			name = "app.outcall.jsonplaceholder.mock.enabled",
			havingValue = "false",
			matchIfMissing = true)
	@Bean("jsonPlaceHolderWebClient")
	public WebClient jsonPlaceHolderWebClient(
			@Value("${app.outcall.jsonplaceholder.url}") String url) {
	...
	public BasePostStub jsonPlaceHolderWireMock(
			@Value("${app.outcall.jsonplaceholder.mock.directory}") String wiremockbaseDirectory,
			@Value("${app.outcall.jsonplaceholder.mock.port:18080}") int wiremockPort) {
		return new BasePostStub(
				new WireMockServer(
						options()
								.port(wiremockPort)
	...
	@ConditionalOnProperty(name = "app.outcall.jsonplaceholder.mock.enabled", havingValue = "true")
	@Bean("jsonPlaceHolderWebClient")
	public WebClient jsonPlaceHolderWebClientStub(
			@Qualifier("jsonPlaceHolderBasePortStub") BasePostStub basePostStub) {
		return WebClient.builder()
